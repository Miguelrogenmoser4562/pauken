/* Note export: markdown file, .doc (Word-compatible HTML), one-click print-to-PDF,
   and review history CSV. */

import type { Block, Note, ReviewLog, QuizQuestion } from "./types";
import { blocksToMarkdown, renderInline, renderMath } from "./markdown";

/** Export review history as CSV: questionId, question, rating, state, due, date.
    Shows one review per row, grouped by question. */
export function exportReviewHistory(
  ...
  const lines = [
    "questionId,question,topic,rating,stateBefore,stateAfter,due,date",
  ];
  for (const log of reviewLogs) {
    const q = qMap.get(log.questionId);
    const question = q ? `"${q.question.replace(/"/g, '""')}"` : "unknown";
    const topic = q?.topic ?? "";
    const date = new Date(log.at).toISOString().slice(0, 10);
    lines.push(
      `${log.questionId},${question},"${topic}",${log.rating},"${log.stateBefore.state}","${log.stateAfter.state}",${new Date(log.stateAfter.due).toISOString().slice(0, 10)},${date}`,
    );
  }
  return lines.join("\n");
}
