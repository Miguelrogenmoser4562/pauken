...
/* Custom block editor: click a block to edit its Markdown source; blur to render.
   ...
   editor has no slash menu, so this is a Pauken differentiator. */
...
export default function BlockEditor({
  ...
  return (
    <div
      className="selectable mx-auto max-w-3xl space-y-2.5 text-base leading-7 break-words"
      onClick={(e) => {
        const cite = (e.target as HTMLElement).closest<HTMLElement>(".source-cite");
        if (cite && onCiteClick) {
          const ref = cite.getAttribute("data-source-ref");
    ...
    >
      {blocks.map((b) => (
        <Row
          ...
          editing={readOnly ? false : editingId === b.id}
          focus={focusId === b.id}
          slashQuery={slash?.id === b.id ? slash.query : null}
          onEdit={() => { if (!readOnly) setEditingId(b.id); }}
          onBlur={() => {
    ...
    case "callout":
      return (
        <div onClick={onEdit} className="flex cursor-text gap-3 rounded-xl bg-callout-bg px-4 py-3.5">
          <span className="text-lg leading-6">{block.emoji ?? <StickyNote className="size-5" />}</span>
