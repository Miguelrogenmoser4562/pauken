...
export default function QuizView({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      setQuestions(qs);
      setAttempts(user?.id ? as.filter((a) => a.userId === user.id || !a.userId) : as);
      setLoaded(true);
    })();
    return () => {
      alive = false;
    };
  }, [repo, note.id, user?.id]);
  ...
  async function handleGenerate() {
    ...
    setGenerating(true);
    try {
      const result = await generateQuiz(engine, note, { count });
...
function QuestionCard({
  ...
  return (
    <div className="rounded-card border border-edge bg-card p-6 shadow-soft">
      ...
      {isAnswered && q.sourcePassage && (
        <SourceCitation
          passage={q.sourcePassage}
          noteId={q.noteId}
          onOpenSource={onOpenSource}
        />
      )}
