...
export default function NoteView() {
  ...
  function handleOpenSource(_noteId: string, start: number, end: number) {
    setModalHighlight({ start, end });
    setModalOpen(true);
  }
    ...
    <div className="flex h-full bg-bg">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        {!note ? (
          ...
          <EditorView note={note} onNote={setNote} onOpenSource={handleOpenSource} />
        ) : view === "chat" ? (
          <Assistant note={note} variant="hero" />
        ) : view === "flashcards" ? (
          <FlashcardsView note={note} onOpenSource={handleOpenSource} />
        ) : view === "quiz" ? (
          <QuizView note={note} onOpenSource={handleOpenSource} />
        ) : null}
      </main>

      {modalOpen && note?.sourceText && (
        <SourceModal
          sourceText={note.sourceText}
          highlightRange={modalHighlight}
          onClose={() => {
            setModalOpen(false);
...
function EditorView({
  ...
  function handleCiteClick(ref: string) {
    const regex = new RegExp(`\\[§${ref.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\]`);
    const match = regex.exec(note.sourceText ?? "");
    if (match) {
      onOpenSource(note.id, match.index, match.index + match[0].length);
    } else {
      onOpenSource(note.id, 0, 0);
    }
  }
  ...
  return (
    <div className="flex h-full">
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`rounded-lg p-2 transition ${editing ? "bg-accent-softer text-accent" : "text-ink-dim hover:bg-card-hover hover:text-ink"}`}
              aria-label={editing ? "View mode" : "Edit note"}
            >
              <PenLine className="size-4.5" />
            </button>
            {note.sourceText && (
              <button
                onClick={() => onOpenSource(note.id, 0, 0)}
                className="rounded-lg p-2 text-ink-dim hover:bg-card-hover hover:text-ink"
        ...
        {note.summary && (
          <div className="mx-6 mb-4 rounded-card border border-edge bg-card shadow-soft">
            <button
              onClick={() => setSummaryOpen((o) => !o)}
              className="flex w-full items-center justify-between px-4 py-2.5 text-sm font-semibold text-ink-dim hover:bg-card-hover"
            >
              <span>Quick Review</span>
              {summaryOpen ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
            </button>
            {summaryOpen && (
              <div className="border-t border-edge px-4 py-3 text-sm text-ink-dim leading-relaxed">
                {note.summary.split("\n").map((line, i) => (
                  <p key={i}>{line}</p>
                ))}
              </div>
